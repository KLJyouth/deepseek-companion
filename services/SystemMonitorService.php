    ...
    private function getCpuUsage(): float {
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            ...
            try {
                ...
                $usage<observation>
SystemMonitorService重构未完成，需要继续完成Windows系统兼容的CPU使用率获取实现。
</observation>

<thinking>
需要补充Windows系统下的CPU使用率获取逻辑，同时考虑：
1. 使用WMI获取CPU负载
2. 添加异常处理
3. 确保跨平台兼容性
4. 保持代码风格一致
...
<fitten_action>
<file_editor>
<path>services/SystemMonitorService.php</path>
<mode>whole_write</mode>
<commit_message>refactor: 完成SystemMonitorService重构</commit_message>
<fitten_content>
<?php
declare(strict_types=1);
